...
contract GovernanceToken is Ownable, ERC20,ILayerZeroReceiver {
  ...
  // send tokens to another chain.
    ...
    function estimateSendTokensFee(uint16 _dstChainId, bytes calldata _toAddress, bool _useZro) external view returns (uint256 nativeFee, uint256 zroFee) {
        // mock the payload for sendTokens()
        bytes memory payload = abi.encode(_toAddress, 1);
        return endpoint.estimateFees(_dstChainId, address(this), payload, _useZro, bytes(""));
    }
